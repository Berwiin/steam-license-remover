...
(async () => {
  ...
  const render = (extraStatus = '') => {
    ...
    if (!status) {
      ...
      else if (isPaused)  status = '⏸ Pozastaveno';
      else if (isWaiting) status = '🔴 Rate limit…';
      else                status = '○ Zastaveno';
    }
    ...
    ui.innerHTML = `
      ...
      <div style="margin-bottom:4px;font-size:13px">
        Odstraněno: <b>${removed}</b>${total > removed ? ` / ~${total}` : ''}
      ...
      <div style="border-top:1px solid #2a3f5a;padding-top:7px">
        <span id="sl-reset" style="cursor:pointer;font-size:11px;color:#8f98a0;opacity:.7">↺ Reset počítadla</span>
      </div>`;
    ...
    if (isWaiting) {
      ...
      cdInterval = setInterval(() => {
        ...
        if (remaining <= 0) {
          ...
          el.textContent = `🔴 Rate limit – čekám ${formatMs(remaining)}`;
        }
      }, 1000);
    }
  };
  ...
  const run = async () => {
    ...
    if (!links.length) {
      setState('stopped');
      ui.innerHTML = `
        <div style="color:#4c6b22;font-weight:bold;font-size:15px;margin-bottom:8px">✅ Hotovo!</div>
        <div>Odstraněno <b>${removed}</b> licencí za ${elapsed()}.</div>
        <div style="color:#8f98a0;font-size:11px;margin-top:6px">Rate limit chyby: ${errors}</div>`;
      return;
    }
